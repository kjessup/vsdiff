...
const { getCommandPrompt } = require('../utils/prompts.js')
...
const createOpenAiCompletion = async (apiKey, model, oneShotPrompt, temperature, maxTokens) => {
  ...
  if (usage.total_tokens && usage.total_tokens >= maxTokens) {
    ...
    The request requires ${usage.total_tokens} tokens and you only have ${maxTokens}. Add more tokens in your AIIDE Settings.`)
  }
...
const createChatCompletion = async ({
  ...
  if (isStreaming) {
    ...
    const systemPrompt = getCommandPrompt(lastMessage, 'chatAIIDE', language)
    const body = JSON.stringify({
      model,
      messages: [
        {
          role: 'system',
          content: systemPrompt// + lastMessage
        },
        {
          role: 'user',
          content: text
